...
// Function to add product from product detail page to cart
function addProductDetailToCart() {
    const addToCartButton = document.querySelector('.add-to-cart');
    if (addToCartButton) {
        addToCartButton.addEventListener('click', () => {
            const productName = document.querySelector('.product-info h2').innerText;
            const productPrice = document.querySelector('.product-info p').innerText.split(': ')[1];
...
// Call the addProductDetailToCart function when the page loads
document.addEventListener('DOMContentLoaded', () => {
    addProductDetailToCart();
    displayCartItems(); // Display cart items on all relevant pages
});
